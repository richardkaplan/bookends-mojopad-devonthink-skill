...
description: "Produce a Bookends-native, deep-linked, highlighted research report on any topic. Creates a Bookends group with subtopic child groups plus a Reports folder, and retrieves sources (guidelines, systematic reviews, key studies) Bookends-first: candidate papers are enumerated by a routine Firecrawl Research / PubMed search per subtopic (verified PMIDs/DOIs), then retrieved via Bookends' search, identifier lookup and PDF download. Attaches full-text PDFs, writes one persistent highlight and a page-accurate bookends:// deep link per source, and assembles ONE combined HTML report (highlighted deep-linked quotes, stance table, narrative synthesis, Vancouver references). Use whenever the user asks for a Bookends research report, a deep-linked or highlighted literature review, evidence synthesis or annotated bibliography in Bookends, or says 'run the Bookends research skill' or 'bookends:// deep-linked quotes'. Depends on the Bookends MCP server that ships inside Bookends.app."
---
...
**Dependencies — nothing to pre-install, and nothing required for the core flow.** The
core pipeline (search → attach → highlight → deep-link → assemble report) uses only the
Bookends MCP and a literature-search MCP — **no Python package, and no PDF library, at
all.** The only Python packages used anywhere are `pyobjc-framework-Quartz` (so an
OPTIONAL validator can SEE a Bookends modal alert) and `pypdf` (link-annotation audit in
an OPTIONAL validator); **those validator scripts install their own packages
automatically at runtime** the first time they run — there is no manual `pip install`
step and none belongs in the install flow. Every other script is standard-library only.
Do not add a manual dependency step back into the install instructions.
...
## Runtime requirements & portability (Claude Code · Cowork · Dispatch)

This skill is designed to run **identically in Claude Code, Claude Cowork, and Dispatch.**
What it needs at RUNTIME is small and the same in all three:

- **REQUIRED — the Bookends MCP** (`mcp__bookends-mcp__*`), which ships inside Bookends.app.
  Every core action goes through it: search, attach PDFs, write the highlight, read back the
  page-accurate `bookends://` deep link, and file the report.
- **REQUIRED for source discovery — a literature-search MCP** (Firecrawl Research tools, or a
  PubMed MCP). Without one, discovery is thin.
- **No Python package, and no PDF library, is required for the core flow.** In particular
  **PyMuPDF / `fitz` is NOT a dependency and is never used** anywhere in this skill —
  highlighting and page-finding are done entirely by the Bookends MCP
  (`bookends_annotate_pdf` writes the highlight; `bookends_get_pdf_content` reads it back with
  its deep link). Do not reintroduce a PDF-highlighting library.

**The bundled `scripts/` (validators, web-share publisher) and `references/bookends.md` are
OPTIONAL developer / QA tooling — not runtime dependencies.** How they travel depends on how
the skill was installed:

- **Full plugin install (Claude Code** — `/plugin marketplace add … && /plugin install …`**):**
  the entire repo is on disk, so the scripts and the reference doc are present. Use them.
- **SKILL.md-only install (Cowork / Dispatch):** these environments save the SKILL.md *text*
  (via `save_skill`) and do **not** carry side files. The `scripts/` and `references/` will
  **not** be present.

**The skill must produce the same report either way. Therefore:**

1. Every reference below to a `scripts/…` validator means: **run it if the file is present;
   if it is absent, perform the equivalent check inline and continue.** For link/attachment
   gates, the inline equivalent is to re-read the annotations with
   `bookends_get_pdf_content { mode: "annotations" }`, confirm each shipped `bookends://` link
   came back from Bookends (never templated), and confirm the report's dual links are present.
2. **A missing validator or a missing `references/bookends.md` NEVER blocks a run** — it only
   downgrades an automated gate to a manual one.
3. All the link forms, MCP calls, and rules the reference doc holds are **also written out in
   this SKILL.md**, so SKILL.md is self-sufficient on its own.

Net effect: full fidelity with automated QA gates when installed as a plugin; the same
report, verified by hand, when only SKILL.md is present.
...
- **Escape ONLY at the HTML boundary (context-correct escaping).** HTML entity escaping
  (`&`→`&amp;`, `<`→`&lt;`, `>`→`&gt;`, `'`→`&#39;`, `"`→`&quot;`) is applied **only** when
  emitting the HTML report body. It must **never** touch a value going to a plain-text sink:
  a Bookends **group name** or any **reference field** (title, keywords, notes set as data),
  a **filename**, or a **DEVONthink** record name. Store every such value as the RAW literal
  (`Guidelines & Systematic Reviews`) and escape a COPY only at render time. The reverse also
  holds — a name taken from HTML/anchor text must be **unescaped** back to the literal
  (`&amp;`→`&`, `&lt;`→`<`, `&gt;`→`>`, `&#39;`/`&apos;`→`'`, `&quot;`→`"`) before it is used
  as a name. Getting this wrong creates groups literally titled *…&amp;…* and breaks the
  report's group deep links (see step 1).
...
- **Group names are PLAIN TEXT — pass the RAW literal name, never an HTML-escaped one.** A
  subtopic called *Guidelines & Systematic Reviews* is created with a real ampersand:
  `name: "<Topic> — Guidelines & Systematic Reviews"`. **Never HTML-escape a value you pass
  to Bookends.** Passing `&amp;` (or `&lt;`, `&gt;`, `&#39;`/`&apos;`, `&quot;`) puts those
  literal character sequences *into* the name — you get a group literally titled
  *Guidelines &amp; Systematic Reviews*. HTML entity escaping belongs ONLY to the HTML
  report body (step 7); it must never touch a name or field sent to Bookends. If you built a
  subtopic name from a list or also use it as an HTML anchor label, keep the stored form
  plain text and escape a COPY only at HTML-render time.
- **Name ↔ deep link must be the SAME literal string.** The report's group link for a
  subtopic is built by percent-encoding this exact clean name (`&`→`%26`, space→`%20`, em
  dash `—`→`%E2%80%94`). So the created group name and every `…/group/<Library>/<encoded
  name>` link that targets it must derive from ONE identical plain-text value. If the group
  is created as *…&amp;…* but the report links to the clean *…&…* (or vice-versa), Bookends
  raises **"Linked group '…' not found"** and the link dead-ends — the exact failure the
  escaping bug causes. Keep one canonical plain-text name per group; encode a copy for the
  link. Every group-open link must resolve to an existing group (verified in step 7).
...
**Retrieval order is Bookends-first, per article — and Bookends does the downloading.**
Bookends runs **on the Mac with unrestricted network access**, whereas a skill sandbox's
network is limited; so routing full-text retrieval through Bookends reaches PDFs a
sandboxed fetch often cannot, and is the more capable path, not merely the more private
one. Bookends can itself resolve identifiers, search the literature databases, and
download and attach full-text PDFs. **Make it the PRIMARY retrieval engine and reach for a
Firecrawl / open-access fetch only when Bookends comes up empty.** For EACH candidate
article, try the Bookends path first; only if it fails do you fall back. **Log which path
each article came through** (Bookends vs Firecrawl-fallback) and carry that provenance
through the run so a completed run is auditable (surface the tally in the step-8
report-back).
...
2. **Bookends online search.** Bookends searches literature databases directly — built-in
   **PubMed, Google Scholar, Semantic Scholar, Google Books, the Library of Congress,
   ...
   IP has access.
3. **Automatic full-text download for a reference — the PRIMARY full-text path.** Once a
   reference carries a PMID / DOI / arXiv id, have **Bookends itself** fetch and attach the
   full-text PDF on the Mac. The first-class call is
   `mcp__bookends-mcp__bookends_add_pdf { items:[{ id }] }` **with neither `path` nor
   `pdf_url`** — passing only the reference id tells Bookends to download the PDF from that
   reference's own PMID/DOI/arXiv id, trying its supported routes and **falling back to
   Unpaywall**. This is the scriptable form of *Refs → Get PDF → From Internet (If
   Available)*; the equivalent AppleScript is `download pdfs {publication item id "<id>"}`
   via `mcp__bookends-mcp__bookends_applescript_run` (see `references/bookends.md` §1).
   Existing PDF attachments are never replaced. Confirm the attachment landed with
...
(and/or PubMed) to locate the article and its **direct PDF URL**, and import the result
**back into Bookends**. **Prefer handing the URL to Bookends so the Mac-side app does the
download, not the sandbox:** `mcp__bookends-mcp__bookends_add_pdf { items:[{ id, pdf_url:
"<direct PDF URL>" }] }`. Only if Bookends cannot fetch that URL do you download the file
yourself and attach the local copy (`bookends_add_pdf { items:[{ id, path }] }`). Mark the
article's provenance **Firecrawl-fallback**. Reserve Firecrawl for this fallback role and
...
- **Ensure a PDF is attached:** a Bookends-path article already has its PDF (step 2A);
  for a Firecrawl-fallback article, **prefer letting Bookends fetch the discovered URL** —
...
- **LAST RESORT — no full text from Bookends, a discovery fetch, OR the open-access sweep (R-BOOKENDS-FULLTEXT-SOURCES-01):** render the abstract to
  ...
  quote is an abstract key sentence, not a full-text deep link). This one-pager is a
  fallback of last resort, never the main retrieval path.
...
For each source, pick a **verbatim, contiguous** key sentence — and pick it **from the
attached PDF's OWN text layer**, not from the source webpage, HTML, or abstract text. Read
the attached PDF's text with `mcp__bookends-mcp__bookends_get_pdf_content { id: "<refID>",
mode: "text" }` and choose the sentence from THAT text, so the `query` you pass exists
byte-for-byte in the exact layer Bookends' `exact` matcher searches. **This is what makes
highlighting reliable — especially for abstract one-pagers:** a sentence copied from the
abstract source frequently does *not* match the Chrome-rendered PDF's text layer (different
whitespace, smart quotes, ligatures, end-of-line hyphenation), so `exact` writes nothing and
the highlight **silently fails on most items** — exactly the "only one highlighted, and it
ended mid-sentence" symptom. Taking the sentence from the PDF's own text layer removes that
mismatch. Then, per quote:
...
matcher — see R-BOOKENDS-EXACT-ONLY-01. Line breaks and most end-of-line hyphenation are
handled by the matcher, but the reliable fix is to take the query **from the PDF's own text
layer** (above); a repeated sentence is disambiguated with `page` (1-based) / `occurrence`.
Ensure **every** article ends with **1–3 highlighted quotes** (the per-article cards each
need 1–3).

**Verify EVERY highlight — never skip silently (R-BOOKENDS-HIGHLIGHT-VERIFY-01).** After
each `bookends_annotate_pdf` call:

1. **Confirm it landed.** Require `annotations_created + annotations_reused ≥ 1` **and**
   `disk_verified: true`. If the result is `0` / not verified, the highlight FAILED — do not
   move on. Re-pick the sentence from the PDF text layer and retry.
2. **Confirm it covers the WHOLE quote.** Read the annotation back with
   `bookends_get_pdf_content { id, mode: "annotations" }` and compare its `text` to your
   intended sentence (normalizing whitespace and end-of-line hyphenation). If the highlighted
   span is **shorter / truncated** — ends mid-sentence — re-highlight with the exact
   contiguous run from the PDF text layer until the readback covers the complete sentence. A
   mid-sentence highlight is a defect, not an acceptable result.
3. **If a passage genuinely cannot be located or highlighted** after retrying, **fail
   loudly:** flag that source in the report as *quote-not-highlightable* (with the reason)
   instead of shipping it as if highlighted, and surface it in the step-8 report-back. Never
   let a swallowed failure pass as success.

When you must render an **abstract one-pager** (the last-resort fallback), render it so its
text layer stays contiguous — left-aligned, **hyphenation disabled**, no justification — so
the key sentence is a single unbroken run the `exact` matcher can find whole.
