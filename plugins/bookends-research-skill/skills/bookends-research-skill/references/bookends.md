...
- **Automatic full-text download for existing references (PRIMARY full-text path):**
  Bookends' native **`download pdfs`** command fetches and attaches the full-text PDF using a
  reference's stored **PMID, DOI, or arXiv id**, trying its supported routes and **falling
  back to Unpaywall**. It runs on the Mac with unrestricted network, so it reaches PDFs a
  sandboxed fetch cannot; existing attachments are never replaced. It is the scriptable form
  of *Refs → Get PDF → From Internet (If Available)*. The first-class call is
  `mcp__bookends-mcp__bookends_add_pdf { items:[{ id }] }` **with neither `path` nor
  `pdf_url`** — passing only the id makes Bookends download from that reference's own
  identifier. (Equivalent AppleScript via `mcp__bookends-mcp__bookends_applescript_run`:)
...
`mcp__mcp-server-firecrawl__firecrawl_research_*` (and/or PubMed) to locate it and its
**direct PDF URL**, then **import the result back into Bookends**: create/locate the
reference (`bookends_quick_add`) and — **preferring to let the Mac-side app do the download** —
attach with `bookends_add_pdf { items:[{ id, pdf_url }] }`, only falling back to a
locally-downloaded `{ id, path }` when Bookends cannot fetch the URL. Mark the article
...
take a real sentence; **never** loosen the mode. **Select the sentence from the attached
PDF's OWN text layer** — `bookends_get_pdf_content { id, mode: "text" }` — so the `query`
matches byte-for-byte. This is what makes Chrome-rendered abstract one-pagers highlightable:
a sentence copied from the source abstract often does **not** match the rendered text layer
(whitespace, smart quotes, ligatures, hyphenation), so `exact` silently writes nothing and
highlighting fails on most items. A repeated sentence is disambiguated with `page` (1-based)
/ `occurrence`.

**Verify EVERY highlight (R-BOOKENDS-HIGHLIGHT-VERIFY-01).** After each call require
`annotations_created + annotations_reused ≥ 1` **and** `disk_verified: true`; then read the
annotation back (`mode: "annotations"`) and confirm its `text` covers the WHOLE intended
sentence, not a truncated span ending mid-sentence — re-highlight from the PDF text layer
until it does. If a passage genuinely cannot be highlighted after retrying, **flag the
source `quote-not-highlightable` and surface it in the report-back** — never skip silently.
Render abstract one-pagers left-aligned with hyphenation disabled, so the sentence is a
single unbroken run the matcher can find whole.
...
- **`bookends_get_pdf_content` echoes highlight text with the line-break hyphen retained**
  (`"first mobi-lized"`). *Matching* is correct; only the echo is un-dehyphenated. When you
  render the quote into the report body, dehyphenate with a regex —
  `re.sub(r"(\w)-\s+(\w)", r"\1\2", text)` — **not** with a PDF library.
